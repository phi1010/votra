...
namespace PhillipKuhrt.VoTra.Classes
{
    public static class DiffProcessor
    {
        ...
        public static Section[] Diff(string a, string b)
        {
            //TODO_L bei mehreren falschen Wörtern

            List<Section> tmp = new List<Section>();
            Difference[] diffs = FindDifferences(a, b);
            int posA = 0, posB = 0;
            foreach (var diff in diffs)
            {
                if (((diff.startA - posA) > 0) || ((diff.startB - posB) > 0))
                {
                    tmp.Add(new Section(
                        a.Substring(posA, diff.startA - posA),
                        b.Substring(posB, diff.startB - posB)
                        ));
                }
                tmp.Add(new Section(
                    a.Substring(diff.startA, diff.countA),
                    b.Substring(diff.startB, diff.countB)
                    ));
                posA = diff.startA + diff.countA;
                posB = diff.startB + diff.countB;
            }
            if (((a.Length - posA) > 0) || ((b.Length - posB) > 0))
            {
                tmp.Add(new Section(
                    a.Substring(posA, a.Length - posA),
                    b.Substring(posB, b.Length - posB)
                    ));
            }
            return tmp.ToArray();
        }
